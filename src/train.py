...
def go(model_name, epochs=50, inputs='./log/*.jpg', limit=None, aug_mult=1, aug_perc=0.0, fine_tuning=False, base_model=None):

    print('working on model', model_name)
    assert(not fine_tuning or base_model is not None)

    '''
    modify config.json to select the model to train.
    '''
    model = models.get_nvidia_model(conf.num_outputs, fine_tuning=fine_tuning, base_model=base_model)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--fine_tuning', type=bool, default=conf.fine_tuning, help='Freeze all but last layer')
    parser.add_argument('--base_model', type=str, default=conf.base_model, help='Model to load weights from if fine-tuning')
    args = parser.parse_args()
    
    go(args.model, epochs=args.epochs, limit=args.limit, inputs=args.inputs, aug_mult=args.aug_mult, aug_perc=args.aug_perc,
        fine_tuning=args.fine_tuning, base_model=args.base_model)
